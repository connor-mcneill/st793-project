...
setting <- function(n, m, B, n_sparse, b2){
  ...
  ## and orthogonal
  ## TODO: explore whether O should actual be blocks
  ## or if it should be result from eigen overall
  } else {
      O = matrix(0,p,p)
      for(i in 1:B){
...
data.nb = function(n, m, p, Gamma, beta, data_gen = 'normal') {
  ...
  X = Z %*% Gamma
  # Generate Y from negative binomial using mean parameterization
  # TODO: Figure out what to plug in for k...
...
set = setting(n, m, B, 4, 0.2)
data = data.nb(n, m, p, set$Gamma, set$beta_1, 'normal')
```
